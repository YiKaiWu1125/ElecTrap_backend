...
with mp_pose.Pose(
  ...
  while cap.isOpened():
    ...
    if results.pose_landmarks:
        x = results.pose_landmarks.landmark[31].x *w#R_DIP].x *w  # 取得食指末端 x 座標
        y = results.pose_landmarks.landmark[31].y *h#mp_hands.HandLandmark.INDEX_FINGER_DIP].y *h  # 取得食指末端 y 座標
        #print(x,y)
        re = 1
    else:
      if(re == 1):
