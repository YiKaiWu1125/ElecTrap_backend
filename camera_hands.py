    ...
    def parse(self, results):
        if results.multi_hand_landmarks:
            for hand_landmarks in results.multi_hand_landmarks:
                # 取得食指末端 x 座標
                self.x = hand_landmarks.landmark[self.mp_hands.HandLandmark.INDEX_FINGER_DIP].x * self.w
                # 取得食指末端 y 座標
                self.y = hand_landmarks.landmark[self.mp_hands.HandLandmark.INDEX_FINGER_DIP].y * self.h
                print(self.x, self.y)
                self.re = 1
        elif self.re == 1:
            print("not hand.")
