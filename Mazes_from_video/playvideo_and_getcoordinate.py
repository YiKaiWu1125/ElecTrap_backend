...
def draw_circle(img, coor_arr, nownumber, half_track_width):
    ...
    if nownumber != -1:
        ...
        img = cv2.circle(
            img,
            (coor_arr[nownumber][0], coor_arr[nownumber][1]),
            half_track_width,
            (col[0], col[1], col[2]),
            -1,
        )
...
def draw_and_save_circle_link_poly(
    img, coor_a, coor_b, index, binarization_arr, open_save, half_track_width
...
def save_crycle_to_3_array(
    binarization_arr, coor_arr, index, width, hight, half_track_width
):
    binarization_arr.append(
        [[0 for j in range(hight + 10)] for i in range(width + 10)]
    )  # [x][y]
    ...
    for i in range(x_begin, x_end):
        for j in range(y_begin, y_end):
            if (
                line_length(i, j, coor_arr[index][0], coor_arr[index][1])
                < half_track_width**2
            ):
        ...
        while cap.isOpened():
            ...
            if not success:
                ...
                for i in range(len(right_hand)):
                    binarization_arr = save_crycle_to_3_array(
                        binarization_arr, right_hand, i, width, hight, half_track_width
                    )
                for i in range(1, len(right_hand)):
                    image, binarization_arr = draw_and_save_circle_link_poly(
                        image,
                        right_hand[i - 1],
                        right_hand[i],
                        i,
