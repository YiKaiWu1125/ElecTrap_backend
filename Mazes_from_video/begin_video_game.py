...
def draw(image, right_hand, binarization_arr, nownumber, half_track_width):
    ...
    for i in range(1, len(right_hand)):
        image = p_g.draw_and_save_circle_link_poly(
            image,
            right_hand[i - 1],
            right_hand[i],
            i,
...
def judgment_status(
    ...
    if sta != "game_over":
        image = draw(
            image, right_hand, binarization_arr, now_number, half_track_width
        )  # 畫出電管
    if sta == "prepare_begin":
        image = cv2.circle(
            image,
            (right_hand[0][0], right_hand[0][1]),
            half_track_width,
            (255, 0, 0),
            -1,
        )  # 畫出起始位置框
    if sta == "playing":
        cv2.circle(
            image,
            (right_hand[len(right_hand) - 1][0], right_hand[len(right_hand) - 1][1]),
            half_track_width,
            (255, 0, 0),
            -1,
        )  # 畫出終止位置框
    if sta == "game_over":
        cv2.putText(
            image,
            "score:" + str(int(game_end_time - game_begin_time)) + " s.",
            (0, 150),
...
def begin_video_game(width, hight, half_track_width, sample_video, video_id):
    ...
    right_hand, binarization_arr = p_g.get_track(
        sample_video, width, hight, half_track_width
    )
    ...
    with mp_pose.Pose(
        ...
        while cap.isOpened():
            ...
            img, now_number, sta, game_begin_time, game_end_time = judgment_status(
                img,
                right_hand,
                binarization_arr,
                x,
            ...
            img = cv2.rectangle(
                img, (x - 10, y - 10), (x + 10, y + 10), (0, 0, 255), 8
            )  # 畫出手的位置
